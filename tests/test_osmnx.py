...
def test_osm_xml() -> None:
    ...
    for filepath in (path_bz2, path_gz_temp, path_osm_temp):
        ...
        ox.graph.validate_graph(G, strict=False)  # non-strict because nodes lack street_count
    ...
    ox.graph.validate_graph(Gc, strict=False)  # non-strict because nodes lack street_count
    _ = etree.parse(fp, parser=parser)
...
def test_save_load() -> None:  # noqa: PLR0915
    ...
    ox.graph.validate_graph(G2, strict=False)  # non-strict because osmid wasn't loaded as int
    ...
    ox.graph.validate_graph(G2, strict=False)  # non-strict because of non-standard types
