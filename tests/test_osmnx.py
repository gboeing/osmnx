...
"""Offline correctness tests for the package."""
...
def test_cache_fixture_manifest(http_cache: Path) -> None:
    """
    Verify the committed raw HTTP cache fixture inventory.

    Parameters
    ----------
    http_cache
        Path to the committed raw HTTP cache fixtures.
    """
...
def test_logging_and_utils(tmp_path: Path) -> None:
    """
    Test logging, timestamps, and citation helpers.
...
def test_geocoder_uses_committed_cache(
    ...
    """
    Test geocoder behavior from raw cached Nominatim responses.
...
def test_graph_downloaders_use_committed_cache(http_cache: Path) -> None:
    """
    Test graph downloader wrappers from raw cached Overpass responses.
...
def test_features_downloaders_use_committed_cache(http_cache: Path) -> None:
    """
    Test feature downloader wrappers from raw cached Overpass responses.
...
def test_stats_simplification_and_conversion(http_cache: Path) -> None:
    """
    Test stats, simplification, and graph/GDF conversion.
...
def test_bearings_routing_and_nearest(http_cache: Path) -> None:
    """
    Test bearings, route solving, and nearest node/edge lookups.
...
def test_elevation_from_cache_and_raster(
    ...
    """
    Test elevation from cached API-style JSON and local rasters.
...
def test_plots_and_colors(http_cache: Path) -> None:
    """
    Test plotting methods with cached graph fixtures.
...
def test_save_load_graph_files(http_cache: Path) -> None:
    """
    Test graph file IO and graph/GDF round trips.
...
def test_osm_xml_read_write(http_cache: Path, tmp_path: Path) -> None:
    """
    Test OSM XML read/write behavior without live HTTP.
...
def test_features_from_xml_and_polygon_holes() -> None:
    """Test local feature XML parsing and polygon hole removal."""
...
def test_utils_geo_projection_and_http_helpers(tmp_path: Path) -> None:
    """
    Test geometry, projection, cache, and HTTP response helpers.
...
_ResponseJson = dict[str, object] | list[dict[str, object]]
...
class _Response(requests.Response):
    ...
    def __init__(self, payload: _ResponseJson, *, ok: bool = True, status_code: int = 200) -> None:
        ...
        self.status_code = status_code if ok or status_code != 200 else 500
...
def test_http_parse_and_request_validation() -> None:
    """Test HTTP response parsing and request parameter validation."""
...
def test_targeted_error_and_helper_branches(
    ...
    """
    Test focused helper branches without live HTTP.
...
def test_validation_warning_and_error_branches() -> None:
    """Test validation warning branches and optional strictness."""
...
def test_uncached_nominatim_request_paths(
    ...
    """
    Test uncached Nominatim request code paths with fake responses.
...
def test_uncached_overpass_and_elevation_request_paths(
    ...
    """
    Test uncached Overpass and elevation request code paths with fake responses.
...
def test_http_cache_and_dns_helper_branches(
    ...
    """
    Test HTTP cache miss and DNS configuration helper branches.
...
def test_overpass_query_and_pause_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    Test Overpass pause parsing and query construction helpers.
...
def test_graph_creation_edge_cases(http_cache: Path) -> None:
    """
    Test graph creation edge cases from cache and synthetic responses.
...
def test_convert_and_io_edge_cases(tmp_path: Path) -> None:
    """
    Test conversion and IO branches with synthetic graphs.
...
def test_simplification_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    Test simplification and consolidation edge cases.
...
def test_consolidation_edge_branches() -> None:
    """Test intersection consolidation edge cases with synthetic graphs."""
...
def test_osm_xml_warning_and_sort_branches(tmp_path: Path) -> None:
    """
    Test OSM XML warnings and way-node topological sorting branches.
...
def test_feature_processing_filter_branches() -> None:
    """Test feature processing, filtering, and relation geometry branches."""
...
def test_projection_stats_distance_and_geometry_branches() -> None:
    """Test local helper branches in projection, stats, distance, and geometry."""
...
def test_truncate_plot_and_routing_branches(tmp_path: Path) -> None:
    """
    Test local helper branches in truncation, plotting, routing, and utilities.
